...
export default function EditCentre(): React.JSX.Element {
  ...
  const [region, setRegion] = useState(''); // "YYYY-MM-DD"
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const url = `${API_BASE}/centres/${id}?ts=${Date.now()}`;
        ...
        console.log(`[EditCenter] Chargement terminé en ${total}ms (cache=${!!cachedStr})`);
      }
    })();
      ...
      <div dir="rtl" style={{ display:'grid', gap:16 }}>
        <div style={styles.toolbar}>
          <div style={styles.toolbarRight}>
            <button onClick={() => nav('/moderator/centres')} style={styles.circleRedBtn} aria-label="رجوع"><ArrowRightIcon/></button>
            <span style={styles.pageTitle}>المركز غير موجودة</span>
          </div>
          <div style={{ width:46, height:46 }} />
        </div>
        <div style={styles.redLine} />
        <div style={{ color:'#b91c1c' }}>❌ لا يوجد مركز بهذا المعرف.</div>
      </div>
    );
  }
    ...
    <div dir="rtl" style={{ display:'grid', gap:16 }}>
      ...
      <div style={styles.toolbar}>
        <div style={styles.toolbarRight}>
          ...
          <span style={styles.pageTitle}>تعديل مركز تدريب</span>
        </div>
        <div style={{ width: 46, height: 46 }} />
      </div>
