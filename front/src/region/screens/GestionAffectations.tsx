...
type Role = 'director_reg' | 'trainer_reg' | 'assistant_reg' | 'coach_reg' | 'trainee';
...
const ROLE_LABEL: Record<Role, string> = {
  director_reg: 'قائد الدورة',
  trainer_reg: 'قيادة الدورة ـ مدرب',
  assistant_reg: 'قيادة الدورة ـ حامل شارة',
  coach_reg: 'المرشد الفني',
  trainee: 'متدرب',
};

export default function GestionAffectationReg(): React.ReactElement | null {
  ...
  const [role, setRole] = React.useState<Role>('director_reg');
  ...
  /* ---------------- Tri des rôles ---------------- */
  const directors = assigns.filter(a => a.role === 'director_reg');
  const trainers = assigns.filter(a => a.role === 'trainer_reg');
  const assistants = assigns.filter(a => a.role === 'assistant_reg');
  const coaches = assigns.filter(a => a.role === 'coach_reg');
      ...
      <div style={styles.card}>
        ...
        <div style={{ display:'grid', gap:8 }}>
          ...
          <div style={{ display:'flex', gap:8 }}>
            ...
            >
              <option value="director_reg">{ROLE_LABEL.director_reg}</option>
              <option value="trainer_reg">{ROLE_LABEL.trainer_reg}</option>
              <option value="assistant_reg">{ROLE_LABEL.assistant_reg}</option>
              <option value="coach_reg">{ROLE_LABEL.coach_reg}</option>
        ...
        {/* ----------- LISTES PAR RÔLE ----------- */}
        <List title={ROLE_LABEL.director_reg}>
          {directors.map(a => (
            <Chip
              key={a.user._id}
        ...
        <List title={ROLE_LABEL.trainer_reg}>
          {trainers.map(a => (
            <Chip
              key={a.user._id}
              label={`${a.user.prenom} ${a.user.nom} — ${ROLE_LABEL[a.role]}`}
        ...
        <List title={ROLE_LABEL.assistant_reg}>
          {assistants.map(a => (
            <Chip
              key={a.user._id}
              label={`${a.user.prenom} ${a.user.nom} — ${ROLE_LABEL[a.role]}`}
        ...
        <List title={ROLE_LABEL.coach_reg}>
          {coaches.map(a => (
            <Chip
              key={a.user._id}
              label={`${a.user.prenom} ${a.user.nom} — ${ROLE_LABEL[a.role]}`}
