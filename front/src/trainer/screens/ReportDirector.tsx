// src/screens/DirectorReport.tsx
...
export default function DirectorReport(): React.JSX.Element {
  ...
  React.useEffect(() => {
    (async () => {
      try {
        ...
        setFormations((list || []).filter(f => f.role === 'director'));
      } catch (e: any) {
  ...
  async function loadReport(fid: string) {
    try {
      ...
      const r = await fetch(
        `${API_BASE}/reports/formations/${fid}/my?role=director&ts=${Date.now()}`,
        {
          headers: headers(),
          cache: 'no-store',
        }
  ...
  async function onSave(fid: string) {
    ...
    try {
      ...
      const r = await fetch(`${API_BASE}/reports/formations/${fid}/my?role=director`, {
        method: 'POST',
        headers: headers(),
        body: JSON.stringify({
          block1: rep.block1,
  ...
  return (
    ...
    >
      ...
      {/* Message général pour le directeur */}
      <div style={{ marginBottom: 12, fontSize: 14, color: '#4b5563' }}>
        هذا الفضاء مخصص لتقرير <strong>قائد الدورة</strong> حول سير الدورة والمتدربين.
      </div>
      ...
      <div style={{ display: 'grid', gap: 10 }}>
        ...
        {!loading && formations.length === 0 && (
          <div style={{ color: '#9ca3af' }}>
            لا توجد دورات أنت مكلّف فيها كقائد دورة.
          </div>
        )}
      </div>
    </div>
